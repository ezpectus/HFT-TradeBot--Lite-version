...
public:
    ...
    bool try_acquire() noexcept {
        ...
        while (expected >= 1.0) {
            if (tokens_.compare_exchange_weak(expected, expected - 1.0,
    ...
    bool try_acquire_n(double n) noexcept {
        ...
        double expected = tokens_.load(std::memory_order_relaxed);
        while (expected >= n) {
            if (tokens_.compare_exchange_weak(expected, expected - n,
                    std::memory_order_relaxed, std::memory_order_relaxed)) {
                return true;
            }
        }
...
public:
    ...
    explicit PreTradeRisk(const Config& cfg)
        : config_(cfg)
        , rate_limiter_(cfg.order_rate_per_second, cfg.order_burst_size)
    {}
    ...
    Result check(
        ...
        if (side == "BUY") new_pos += quantity;
        else new_pos -= quantity;
    ...
    void update_daily_pnl(double pnl) noexcept {
        daily_pnl_.store(pnl, std::memory_order_relaxed);
    }
