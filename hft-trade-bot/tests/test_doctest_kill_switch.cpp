...
// Tests: activate/deactivate, reason recording, timestamp recording, callbacks, can_trade, double-activate
...
TEST_CASE("KillSwitch: notify callback invoked with correct reason") {
    ...
    ks.set_notify_callback([&](KillSwitch::Reason r) {
        notified_reason.store(static_cast<int>(r));
    });
    ks.activate(KillSwitch::Reason::MARGIN_CALL);
